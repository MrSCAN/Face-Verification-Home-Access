...
try
{
    ...
    for (size_t cluster_id = 0; cluster_id < num_clusters; ++cluster_id)
    {
        ...
        // Generate the file name for each cluster
        std::string file_name = "face_cluster_" + std::to_string(cluster_id) + ".png";
        save_png(tile_images(temp), file_name);
        cout << "Saved face cluster " << cluster_id << " as " << file_name << endl;
